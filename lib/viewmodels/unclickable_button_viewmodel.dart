...
class UnclickableButtonViewModel extends ChangeNotifier {
  ...
  void _checkSpeedThreshold() {
    if (_buttonSpeed.value > 2.0 &&
        _movementStrategy is BasicMovementStrategy) {
      _movementStrategy = AdvancedMovementStrategy(this);
    } else if (_buttonSpeed.value > 3.0 &&
        !(_movementStrategy is TrollMovementStrategy)) {
  ...
  void _checkIfCornered(Offset position) {
    ...
    final isNearRightEdge =
        position.dx > _screenSize.width - buttonWidth - edgeThreshold;
    final isNearTopEdge = position.dy < edgeThreshold;
    final isNearBottomEdge =
        position.dy > _screenSize.height - buttonHeight - edgeThreshold;

    _isCornered =
        (isNearLeftEdge && (isNearTopEdge || isNearBottomEdge)) ||
...
class TrollMovementStrategy extends MovementStrategy {
  ...
  Offset _teleportMove(Offset current, Size screenSize, Size buttonSize) {
    ...
    if (isLeftSide) {
      newX =
          screenSize.width / 2 +
          _random.nextDouble() * (maxX - screenSize.width / 2);
    } else {
    ...
    if (isTopSide) {
      newY =
          screenSize.height / 2 +
          _random.nextDouble() * (maxY - screenSize.height / 2);
    } else {
...
class CircularMovementPattern extends MovementPattern {
  ...
  Offset getNextPosition({
    ...
    return ensureWithinBounds(Offset(newX, newY), screenSize, buttonSize);
  }
}
  ...
  Offset getNextPosition({
    ...
    if (isHorizontal) {
      ...
      newX =
          ...
          direction *
              (minDistance +
                  _random.nextDouble() * (maxDistance - minDistance));
      newY =
          currentPosition.dy +
          (moveCount % 2 == 0 ? 1 : -1) * (40 + _random.nextDouble() * 40);
    } else {
